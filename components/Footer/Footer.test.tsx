...
describe('pied de page', () => {
  ...
  it('affiche les liens', () => {
    ...
    const nosPartenaires = within(liensJusticePlurielle[2]).getByRole('link', { name: wording.NOS_PARTENAIRES })
    expect(nosPartenaires).toHaveAttribute('href', paths.NOS_PARTENAIRES)
    const politiqueDeConfidentialite = within(liensJusticePlurielle[3]).getByRole('link', { name: wording.POLITIQUE_DE_CONFIDENTIALITE })
    expect(politiqueDeConfidentialite).toHaveAttribute('href', paths.POLITIQUE_DE_CONFIDENTIALITE)
    const foireAuxQuestions = within(liensJusticePlurielle[4]).getByRole('link', { name: wording.FOIRE_AUX_QUESTIONS })
